...
contract WeightedTreasuryVaultTest is Test {
    ...
    function test__Rebalance() public {
        ...
        // Send some ETH to the vault
        vm.deal(address(vault), 5 ether);
        
        // Send some BTC to the vault
        btc.mint(address(vault), 2 * 10**8); // 2 BTC

        // Send some USDC to the vault
        usdc.mint(address(vault), 10000 * 10**6); // 10,000 USDC

        // Create rebalance data
        // This would typically be generated by the swap adapter
        ...
        emit VaultState(2, block.timestamp, 0, 0, new uint256[](0), address(0)); // We don't check exact values
    ...
    function test__Rebalance_CheckStateEvent() public {
        // Fund the vault
        vm.deal(address(vault), 5 ether);
        btc.mint(address(vault), 2 * 10**8);
        usdc.mint(address(vault), 10000 * 10**6);
        
        // Expect an event with stateId = 2 (1 from constructor, 2 from this rebalance)
        vm.expectEmit(true, false, false, false);
        emit VaultState(2, 0, 0, 0, new uint256[](0), address(0));
    ...
    function test__RebalanceDepositWithdraw() public {
        // Fund the vault
        vm.deal(address(vault), 5 ether);
        btc.mint(address(vault), 2 * 10**8);
        usdc.mint(address(vault), 10000 * 10**6);
        ...
        uint256 receivedAssets = vault.withdraw(withdrawShares, user, user);
        vm.stopPrank();
        
        // Verify withdrawal
        assertEq(receivedAssets, expectedAssets, "Incorrect assets returned");
        assertEq(vault.balanceOf(user), shares - withdrawShares, "Incorrect remaining shares");
    }
